...
static void singleton_check ( void ) 
{
    ...
    // Singleton
    if ( initialized == 0 )
        bump_construct((g_bump *)&sticky_allocator, (void *)0x2FFFF0, (2 << 20) - sizeof(g_bump_t)),
        bump_construct((g_bump *)&allocator       , (void *)0x4FFFF0, (2 << 20) - sizeof(g_bump_t));
...
int sticky_alloc ( void **pp_result, size_t size )
{
    ...
    // Allocate memory
    bump_alloc(sticky_allocator, pp_result, size);

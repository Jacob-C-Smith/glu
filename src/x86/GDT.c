...
{
    [SEGMENT_ACCESS_CODE_EQUAL_PRIVILEGE] = "equal privil",
    [SEGMENT_ACCESS_CODE_LOWER_PRIVILEGE] = "lower privil"
};

static const char *gdt_access_code_read_write_strings [] = 
{
    [SEGMENT_ACCESS_CODE_READ_LOCK]  = "readlock",
...
static const char *gdt_access_data_read_write_strings [] = 
{
    ...
    [SEGMENT_ACCESS_DATA_WRITE_ALLOW] = "readwrite"
};
...
int gdt_segment_descriptor_print ( gdt_segment_descriptor *p_gdt_segment_descriptor )
{
    ...
    format("\trange  : [ 0x%x - 0x%x ]\n", p_gdt_segment_descriptor->p_base,
        ...
        (size_t)p_gdt_segment_descriptor->p_limit 

        // NOTE: Read this from the inside out. 8 -> 1 -> 12

        // Correct the upper bound according to the granuliarty of the segment 
        // descriptor 
        << (

            // 0    if byte granulairty (no effect)
            // 12   if page granularity (page addresses are aligned such that)
            12 * 
            ( 

                // 1   if 4096-byte granularity
                // 0   if    1-byte granularity
                0 < (

                    // 8   if 4096-byte granularity
                    // 0   if    1-byte granularity
                    flags & GDT_FLAG_GRANULARITY_PAGE
                )
            )
        )
    );
